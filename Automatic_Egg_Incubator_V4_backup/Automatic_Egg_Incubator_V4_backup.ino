...
const long intSwing = 3600000;            //Swing delay, in milliseconds. 1000 millis = 1 second. 7200000 millis = 2 hours
...
void swing(){
  ...
  if (currSwing - prevSwing >= intSwing){
    ...
    if (digitalRead(limSwitch_left) == LOW){
      digitalWrite(rel4, HIGH);
      delay(500);
      analogWrite(motor, motor_speed_right);
      left_activated = true;
    }
    if (digitalRead(limSwitch_right) == LOW){
      digitalWrite(rel4, LOW);
      delay(500);
      analogWrite(motor, motor_speed_left);
...
void minusPressed(){
  ...
  if (digitalRead(button2) == LOW){
      ...
      if (!manual){
        ...
        if (setUp == 5){
          motor_speed_left--;
          if (motor_speed_left < 1){
            motor_speed_left = 254;
          }
        }
        if (setUp == 6){
          motor_speed_right--;
          if (motor_speed_right < 1){
            motor_speed_right = 254;
          }
        }
      }
    }
  ...
  if (digitalRead(button3) == LOW){
    if (currAdd - prevAdd >= intAdd){
      ...
      if (!manual){
        ...
        if (setUp == 5){
          motor_speed_left++;
          if (motor_speed_left > 254){
            motor_speed_left = 1;
          }
        }
        if (setUp == 6){
          motor_speed_right++;
          if (motor_speed_right > 254){
            motor_speed_right = 1;
          }
        }
      }
...
void display_setup(){
  ...
  if (currDispSet - prevDispSet >= intDispSet){
    ...
    if (setUp == 5){
      digitalWrite(LED4, LOW);
      digitalWrite(LED5, HIGH);
      sevseg.setNumber(motor_speed_left, 0);
    }
    if (setUp == 6){
      digitalWrite(LED5, LOW);
      digitalWrite(LED6, HIGH);
      sevseg.setNumber(motor_speed_right, 0);
    }
  }
}
